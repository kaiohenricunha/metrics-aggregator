...
if [[ ! -s "$EVIDENCE_DIR/metrics-raw.txt" ]]; then
  ...
  if echo "$PROMTOOL_OUTPUT" | grep -vE '(^[^:]+: no help text for metric:|^[^ ]+ no help text$)' | grep -q '[^[:space:]]'; then
...
if [[ -s "$EVIDENCE_DIR/istio-permissive-metrics.txt" ]]; then
  ...
  if echo "$ISTIO_PROMTOOL_OUTPUT" | grep -vE '(^[^:]+: no help text for metric:|^[^ ]+ no help text$)' | grep -q '[^[:space:]]'; then
